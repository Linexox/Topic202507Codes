...
class UGNN_ete(MultiModalEndtoEndRecommender):
    def __init__(self, config, dataset):
        super(UGNN_ete, self).__init__(config, dataset)
        self.config = config
    ...
    def __init_mm_feature(self, dataset):
        ...
        # else:
        #     self.a_feat = None
        self.item_feat_dim = self.latent_dim * 3 #self.latent_dim * 3
    ...
    def compute(self, train=True):
        ...
        if train:
            self.v_emb_u, self.v_emb_i = self.sgl_encoder(users_emb, self.v_train_feat)
        else:
            self.v_emb_u, self.v_emb_i = self.sgl_encoder(users_emb, self.v_test_feat)
        ...
        if self.config["dataset"] == "kwai":
            ...
            if self.t_train_feat is not None or self.t_test_feat is not None:
                # self.t_dense_emb = self.t_dense(self.t_feat)  # t=>id
                if train:
                    self.t_emb_u, self.t_emb_i = self.sgl_encoder(users_emb, self.t_train_feat)
                else:
                    self.t_emb_u, self.t_emb_i = self.sgl_encoder(users_emb, self.t_test_feat)
    ...
    def modality_edge_dropout_emb(self, u_ids, pos_ids, neg_ids):
        ...
        if self.config["dataset"] == "kwai":
            ...
            v_dense = self.v_train_feat
            t_dense = self.t_train_feat
    ...
    def calculate_loss(self, interaction):
        ...
        if self.config['using_vl_loss']:
            return main_loss + self.config['ssl_alpha'] * ssl_loss + self.vl_loss
    ...
    def full_sort_predict(self, interaction, candidate_items=None):
        ...
        users = interaction[0]
        # update user and item embeddings base on test features
        self.user_embeddings, self.item_embeddings = self.compute(train=False)
