...
from models.beit3.modeling_utils import BEiT3Wrapper, _get_base_config, _get_large_config
...
class BEiT3ForRecsys_retrieval_style(BEiT3Wrapper):
    def __init__(
            self,
            args,
            **kwargs
    ):
        super(BEiT3ForRecsys, self).__init__(args=args)
        embed_dim = args.encoder_embed_dim
        self.language_head = nn.Linear(embed_dim, embed_dim, bias=False)
        self.vision_head = nn.Linear(embed_dim, embed_dim, bias=False)
        self.language_head.apply(self._init_weights)
...
class BEiT3ForRecsys(BEiT3Wrapper):
    def __init__(
            self,
            args,
            norm_layer=nn.LayerNorm,
        ...
        super(BEiT3ForRecsys, self).__init__(args=args)
        embed_dim = args.encoder_embed_dim
        self.img_pooler = Pooler(
            input_features=embed_dim,
            output_features=embed_dim,
            norm_layer=norm_layer,
        )
        self.img_pooler.apply(self._init_weights)

        self.txt_pooler = Pooler(
            input_features=embed_dim,
            output_features=embed_dim,
            norm_layer=norm_layer,
        )
        self.txt_pooler.apply(self._init_weights)
    ...
    def forward(self, image, text_description, padding_mask, item_id=None, only_infer=True, **kwargs):
        outputs = self.beit3(
            textual_tokens=text_description,
            visual_tokens=image,
            text_padding_position=padding_mask,
        ...
        x = outputs["encoder_out"]
        img_cls_rep = self.img_pooler(x[:, 0:198, :])
...
@register_model
def beit3_base_patch16_224_recsys(pretrained=False, **kwargs):
    args = _get_base_config(**kwargs)
    model = BEiT3ForRecsys(args, **kwargs)
    return model
