...
class HollaRecDataLoader(BaseDataLoader):
    ...
    def rec_process_fn(self):
        ...
        """
        augment_dataset = []
        for conv in tqdm(self.dataset, desc="[Rec process]"):
    ...
    def conv_process_fn(self):
        ...
        """
        dataset = []
        for conv in tqdm(self.dataset, desc="[Conv process]"):
            if conv["role"] == "Recommender":
                dataset.append(conv)
        return dataset
    ...
    def conv_batchify(self, batch):
        ...
        for conv in batch:
            ...
            # 3. 收集电影ID
            batch_context_movies.append(conv.get("context_movies", []))
