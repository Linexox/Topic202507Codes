...
class GraphLlamaModel(LlamaModel):
    ...
    def forward(
        ...
        if inputs_embeds is None: # 如果调用方没有显式传 inputs_embeds，函数会用模型的词嵌入层对 input_ids 进行查表
        ...
        if graph_tower is not None and (input_ids.shape[1] != 1 or self.training) and graph_data is not None:
            ...
            # Graph Tower前向计算， 得到图节点特征graph_node_features
            with torch.no_grad(): 
            ...
            if type(graph_data) is list:
                ...
                # NOTE: graph_node_features: List[torch.FloatTensor[num_nodes, graph_hidden_size]]
                # 对每一个图进行投影为LLM的隐藏维度
            ...
            for cur_input_ids, cur_input_embeds in zip(input_ids, inputs_embeds):
                if (cur_input_ids == graph_tower.config.graph_patch_token).sum() == 0: # 若没有图占位符graph_patch_token
                    ...
                    cur_graph_idx += 1
                    continue
                if graph_tower.config.use_graph_start_end: # 若使用图的起止标记
                    cur_graph_features = graph_node_features[cur_graph_idx]
                    num_patches = cur_graph_features.shape[0]
                    if (cur_input_ids == graph_tower.config.graph_start_token).sum() != (cur_input_ids == graph_tower.config.graph_end_token).sum(): # 起止标记数量不等，报错
                        raise ValueError("The number of graph start tokens and graph end tokens should be the same.")
                    graph_start_tokens = torch.where(cur_input_ids == graph_tower.config.graph_start_token)[0] # 找到所有起始标记的位置
                    # NOTE: torch.where return tuple : [ [indices,], ... ] , shape : dim*num_graphs_start_tokens
                    ...
                    for graph_start_token_pos in graph_start_tokens:
                        ...
                        if orig_embeds_params is not None: # 若使用了LLaVA的预训练嵌入，则保持这些嵌入不变
...
class GraphLlamaForCausalLM(LlamaForCausalLM):
    ...
    def forward(
        ...
        past_key_values: Optional[List[torch.FloatTensor]] = None,
        inputs_embeds: Optional[torch.FloatTensor] = None,
        labels: Optional[torch.LongTensor] = None, # shape: (batch_size, seq_len)
        use_cache: Optional[bool] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        # graph_node_reps: Optional[torch.FloatTensor] = None,
        ...
        # decoder output    s consists of (dec_features, layer_state, dec_hidden, dec_attn)
        outputs = self.model(
            input_ids=input_ids,
