...
class HReDialDataset(BaseDataset):
    ...
    def _load_raw_review(self):
        # load train/valid/review data
        train_path = open(os.path.join(self.dpath, 'train_conv_idx_to_review_info.pkl'), "rb")
        train_review = pkl.load(train_path)
        logger.debug(f"[Load review data from {train_path}]")
        test_path = open(os.path.join(self.dpath, 'test_conv_idx_to_review_info.pkl'), "rb")
        test_review = pkl.load(test_path)
        logger.debug(f"[Load review data from {test_path}]")
        valid_path = open(os.path.join(self.dpath, 'valid_conv_idx_to_review_info.pkl'), "rb")
        valid_review = pkl.load(valid_path)
        logger.debug(f"[Load review data from {valid_path}]")

        return train_review, test_review, valid_review
    ...
    def _raw_data_process(self, raw_data):
        ...
        augmented_convs = [
            [
                self._merge_conv_data(conv["dialog"], conv["user_id"], conv["conv_id"]) 
                for conv in convs
            ] 
            for convs in tqdm(raw_data)
        ]
        augmented_conv_dicts = []

        for conv_list in tqdm(augmented_convs): # conv_list中存储了一个用户的所有对话
            # 获取最近的实体列表
            related_entities = list()
            for conv in conv_list[:-1]:
                current_related_entities = set() # 每次循环都重新初始化为空集合
                for uttr in conv:
            ...
            # NOTE: related_entities长度为len(conv_list) - 1， 每一个元素对应一轮对话实体列表
            # 获取最近的电影
            related_items = list()
            for conv in conv_list[:-1]:
                current_related_items = set() # 每次循环都重新初始化为空集合
                for uttr in conv:
                    for item_id in uttr['movie']:
            ...
            for i in range(len(augmented_conv_list)): # 由最后一次对话增强的样本都使用相同的历史信息
        ...
        for i in tqdm(range(len(augmented_conv_dicts))):
            extended_items = self._search_extended_items(
                augmented_conv_dicts[i]['conv_id'],
                augmented_conv_dicts[i]['context_items'])
    ...
    def _augment_and_add(self, raw_conv_dict): # 最后一个对话
        ...
        for i, conv in enumerate(raw_conv_dict):
            ...
            if len(context_tokens) > 0: # 若有上文
                ...
                augmented_conv_dicts.append(conv_dict) # 生成一个训练样本
            # 更新上下文信息
