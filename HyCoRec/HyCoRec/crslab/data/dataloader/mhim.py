...
class MHIMDataLoader(BaseDataLoader):
    ...
    def __init__(self, opt, dataset, review_data, vocab):
        """
        ...
        """
        super().__init__(opt, dataset, review_data)
    ...
    def rec_process_fn(self):
        """遍历原始对话数据，将每个推荐项（如电影）拆分为单独的样本，生成增强后的推荐数据集。"""
        augment_dataset = []
    ...
    def _get_review(self, batch_conv_id):
        review = {
            'selected_infoListListInt':[],
            'selected_entityIds':[],
        }
        for conv_idx in batch_conv_id:
            if str(conv_idx) in self.review_data:
                temp_review = self.review_data[str(conv_idx)]
                selected_infoListListInt = temp_review["selected_infoListListInt"]
                selected_entityIds = temp_review["selected_entityIds"]
            else:
                selected_infoListListInt = [[]]
                selected_entityIds = []
            review["selected_infoListListInt"].append(selected_infoListListInt)
            review["selected_entityIds"].append(selected_entityIds)
        return review
    ...
    def rec_batchify(self, batch):
        ...
        review = self._get_review(batch_conv_id)
