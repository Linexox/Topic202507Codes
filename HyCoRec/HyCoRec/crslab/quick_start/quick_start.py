...
def run_crslab(config, save_data=False, restore_data=False, save_system=False, restore_system=False,
    ...
    if isinstance(config['tokenize'], str):
        ...
        train_dataloader = get_dataloader(config, CRS_dataset.train_data, CRS_dataset.train_review, vocab)
        valid_dataloader = get_dataloader(config, CRS_dataset.valid_data, CRS_dataset.valid_review, vocab)
        test_dataloader = get_dataloader(config, CRS_dataset.test_data, CRS_dataset.test_review, vocab)
    else:
        tokenized_dataset = {}
        train_dataloader = {}
        valid_dataloader = {}
        ...
        for task, tokenize in config['tokenize'].items():
            ...
            train_dataloader[task] = get_dataloader(config, train_data, train_review, vocab[task])
            valid_dataloader[task] = get_dataloader(config, valid_data, valid_review, vocab[task])
            test_dataloader[task] = get_dataloader(config, test_data, test_review, vocab[task])
