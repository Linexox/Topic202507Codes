...
class MHIMModel(BaseModel):
    ...
    def _get_conceptnet_embedding(self, hypergraph_items, raw_conceptnet_embedding):
    ...
    # 注意力融合特征向量
    def _attention_and_gating(self, session_embedding, knowledge_embedding, conceptnet_embedding, context_embedding, review_embedding):
        # related_embedding = torch.cat((session_embedding, knowledge_embedding, conceptnet_embedding, review_embedding), dim=0)
    ...
    # 获取用户编码
    def encode_user(self, batch_related_entities, batch_related_items, batch_context_entities, review, kg_embedding, con_embedding):
        # print(review)
        user_repr_list = []
        for session_related_entities, session_related_items, context_entities in zip(batch_related_entities, batch_related_items, batch_context_entities):
            ...
            _, conceptnet_hyper_edge_index = self._get_conceptnet_hypergraph(session_related_entities)
            raw_conceptnet_embedding = self.hyper_conv_conceptnet(con_embedding, conceptnet_hyper_edge_index)
            conceptnet_embedding = self._get_conceptnet_embedding(hypergraph_items, raw_conceptnet_embedding)

            review_token = self._get_review_token(review['selected_infoListListInt'])
            review_state = self.review_encoder(review_token)[0]
            review_embedding = self.review_decoder(review_state)
            # review_embedding = None

            # print(review_token, review_state, review_embedding)

            # 注意力机制
            if len(context_entities) == 0:
                user_repr = self._attention_and_gating(session_embedding, knowledge_embedding, conceptnet_embedding, None, review_embedding)
            else:
                context_embedding = kg_embedding[context_entities]
                user_repr = self._attention_and_gating(session_embedding, knowledge_embedding, conceptnet_embedding, context_embedding, review_embedding)
            user_repr_list.append(user_repr)
    ...
    # 获取超图后数据
    def encode_session(self, batch_related_entities, batch_related_items, batch_context_entities, kg_embedding, con_embedding, review):
        """
        ...
        for session_related_entities, session_related_items, context_entities in zip(batch_related_entities, batch_related_items, batch_context_entities):
            ...
            # 数据拼接
            if len(context_entities) == 0:
                session_repr = torch.cat((session_embedding, knowledge_embedding, conceptnet_embedding, review_embedding), dim=0)
                session_repr_list.append(session_repr)
            else:
                context_embedding = kg_embedding[context_entities]
                session_repr = torch.cat((session_embedding, knowledge_embedding, context_embedding, conceptnet_embedding, review_embedding), dim=0)
                session_repr_list.append(session_repr)
