...
class TGReDialDataset(BaseDataset):
    ...
    def __init__(self, opt, tokenize, restore=False, save=False):
        ...
        if self.replace_token:
            if self.replace_token_idx:
                ...
                self.side_data["embedding"] = np.insert(
                    self.side_data["embedding"],
                    len(self.side_data["embedding"]),
                    self.side_data["embedding"][0],
                    axis=0,
                )
    ...
    def _load_other_data(self):
        ...
        self.entity2id = json.load(
            open(os.path.join(self.dpath, "entity2id.json"), encoding="utf-8")
        )  # {entity: entity_id}
    ...
    def _convert_to_id(self, conversation):
        ...
        for utt in conversation["messages"]:
            ...
            if self.replace_token:
                if len(utt["movie"]) != 0:
                    while "《" in utt["text"]:
                        ...
                        utt["text"] = (
                            utt["text"][:begin] + [self.replace_token] + utt["text"][end + 1 :]
                        )
            ...
            entity_ids = [
                self.entity2id[entity] for entity in utt["entity"] if entity in self.entity2id
            ]
            ...
            final_kws = [
                self.topic2ind[kw] if kw is not None else self.pad_topic_idx
                for kw in utt["final"][1]
            ]
            ...
            user_profile = [
                [self.tok2ind.get(token, self.unk_token_idx) for token in sent]
    ...
    def _augment_and_add(self, raw_conv_dict):
        ...
        for i, conv in enumerate(raw_conv_dict):
            text_tokens, entities, movies, words, policies = (
                conv["text"],
                conv["entity"],
                conv["movie"],
                conv["word"],
    ...
    def _entity_kg_process(self):
        ...
        return {"edge": list(edges), "n_relation": len(relation2id), "entity": list(entities)}
