...
class ReDialDataset(BaseDataset):
    ...
    def _raw_data_process(self, raw_data):
        augmented_convs = [
            self._merge_conv_data(conversation["dialog"]) for conversation in tqdm(raw_data)
        ]
    ...
    def _merge_conv_data(self, dialog):
        ...
        for utt in dialog:
            ...
            movie_ids = [
                self.entity2id[movie] for movie in utt["movies"] if movie in self.entity2id
            ]
            entity_ids = [
    ...
    def _augment_and_add(self, raw_conv_dict):
        ...
        for i, conv in enumerate(raw_conv_dict): # conv(uttrance)
